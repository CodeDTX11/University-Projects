##########################################################################
# Dylan Messerly, CS 2318-002, Assignment 2 Part 1 Program B
##########################################################################
...
main:
		...
		# instruction so that the program will work just like 
		...
		li $v0, 1
		and $a0, $t1, $t0	# $a0 gets "all bits of x cleared except the rightmost 1"
		syscall
		
		nor $t2, $a0, $0	# $t2 gets mask2 that is "$a0 with all its bits toggled"
		
		li $v0, 4
		la $a0, outLab2        
		syscall			# print output label 2
		li $v0, 1
		and $a0, $t2, $t0	# $a0 gets "all bits of x with the rightmost 1 cleared"
		syscall
